    ...
    try {
      ...
      if (isFollowing) {
        ...
        batch.set(
          _firestore.collection('users').doc(widget.userId).collection('followers').doc(currentUserId),
          {
            'username': widget.currentUser.displayName ?? widget.currentUser.email?.split('@').first ?? 'user',
            'timestamp': FieldValue.serverTimestamp(),
          },
        );
        batch.update(
...
class _FollowListPageState extends State<FollowListPage> {
    ...
    try {
      ...
      final usersData = await Future.wait(
        querySnapshot.docs.map((doc) async {
          // Get the user data from the main users collection
          final userDoc = await _firestore.collection('users').doc(doc.id).get();
          String username;

          if (widget.currentUser.uid == doc.id) {
            // Current user
  ...
  Future<void> _toggleFollowStatus(String targetUserId, int index) async {
    if (widget.currentUser.uid == targetUserId) {
      // Show a message that you can't follow yourself
    ...
    try {
      final isFollowing = filteredUsers[index]['isFollowing'];
      final batch = _firestore.batch();

      if (isFollowing) {
        batch.delete(
          _firestore.collection('users').doc(widget.currentUser.uid).collection('following').doc(targetUserId),
        );
        batch.delete(
          _firestore.collection('users').doc(targetUserId).collection('followers').doc(widget.currentUser.uid),
        );
        batch.update(
      ...
      if (mounted) {
        setState(() {
          filteredUsers[index]['isFollowing'] = !isFollowing;
          // Also update the main users list
          final mainIndex = users.indexWhere((u) => u['id'] == targetUserId);
          if (mainIndex != -1) {
            users[mainIndex]['isFollowing'] = !isFollowing;
          }
        });
      }
    } catch (e) {
  ...
  void _viewUserProfile(String userId) {
    if (userId == widget.currentUser.uid) return;
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: widget.currentUser.uid != user['id']
                    ...
                    onPressed: () => _toggleFollowStatus(user['id'], index),
                  )
                      : null,
