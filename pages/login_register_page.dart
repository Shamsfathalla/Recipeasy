...
class _LoginPageState extends State<LoginPage> {
  ...
  Widget _entryField(String hintText, TextEditingController controller, IconData icon, bool isPassword) {
    return TextField(
  ...
  Widget _errorMessage() {
    return Padding(
      ...
      child: Text(
        errorMessage,
        style: TextStyle(
          color: errorMessage.contains('success') ? Colors.green : Colors.red, // Green for success, red for errors
          fontSize: 14,
        ),
    ...
    return TextButton(
      onPressed: () {
        setState(() {
          ...
          _controllerForgotPasswordEmail.text = _controllerEmail.text; // Copy email from Sign In to Forgot Password
        });
      },
      child: const Text(
        'Forgot Password?',
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    ...
                    setState(() {
                      forgotPasswordMessage = 'Password reset link sent! Check your inbox.';
                    });
                  } on FirebaseAuthException catch (e) {
                    if (e.code == 'user-not-found') {
                      setState(() {
                        forgotPasswordMessage = 'Error: No user found with this email.';
                      });
                    } else {
                      setState(() {
      ...
      body: Stack(
        children: [
          ...
          AnimatedPositioned(
            duration: const Duration(milliseconds: 500),
            curve: Curves.easeInOut,
            top: showForm && !isLogin ? screenHeight * 0.13 : screenHeight * 0.19, // Adjusted position
            left: 0,
            right: 0,
            child: Center(
              child: Image.asset(
          ...
          Stack(
            ...
            children: [
                  ...
                  onTap: () {
                    setState(() {
                      if (showForgotPassword) {
                        ...
                        showForm = false; // Close sign-in/sign-up box
                        showForgotPassword = false; // Ensure forgot password box is also closed
                        errorMessage = '';
              ...
              Positioned(
                top: screenHeight * 0.51, // Adjusted position below the welcome text
                left: 0,
                right: 0,
                child: Visibility(
